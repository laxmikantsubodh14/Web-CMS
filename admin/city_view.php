...
if($_REQUEST['del']=='yes')
{
            $sqlDel="delete from imp_city where city_id='$_REQUEST[id]'";
...
if(is_post_back()) {
	...
	if(is_array($bb)) {
		$bbb = implode(',', $bb);		
		if($_REQUEST['Delete']!='') {
			$sqlDel="delete from  imp_city where city_id  in ($bbb)";
			$resultDel = $contact->Execute($sqlDel) or die($contact->ErrorMsg());
			...
			$sql = "update  imp_city   set Status= 'Y' where city_id   in ($bbb)";
			...
			$sql = "update  imp_city set Status= 'N' where city_id  in ($bbb)";
			$resultsql = $contact->Execute($sql) or die($contact->ErrorMsg());
...
$columns = "select * ";
$sql = " from  imp_city  where 1 ";
$sql = apply_filter($sql, $adm_login_id, $adm_login_id_filter,'city_id');
...
<table width="100%" border="0" cellpadding="4" cellspacing="1" class="tblBackColor">
  <tr bgcolor="#FFFFFF"><td><table width="100%" border="0" cellpadding="0" cellspacing="0" style="BACKGROUND-IMAGE: url(images/tablebgrep.gif)">
    <tr><td width="3%" class="pageHead"><img height="17" alt="" 
                ...
                vspace="3" /></td>
    <td width="97%" align="left"><div id="pageHead">City details View</div></td></tr></table>
</td></tr>
...
<tr><td height="250" valign="top"><br>
       <div align="right"> <a href="city_add.php">Add City Information</a></div>
...
<?php } else{ ?>
  ...
  <table width="100%"  border="0" cellpadding="3" cellspacing="1" bgcolor="#d8d8d8" class="tableList" >
    <tr>
	      <th width="8%" nowrap> SNo. <?php echo sort_arrows('city_id')?></th>
              <th width="17%" nowrap>Country <?php echo sort_arrows('country_id')?></th>
			  <th width="17%" nowrap> State <?php echo sort_arrows('StatesID')?></th>
			  <th width="17%" nowrap> City <?php echo sort_arrows('city')?></th>
...
?>   <tr class="<?php echo $css?>">
      	      <td nowrap><?php echo $result->fields['city_id']?> </td>
			...
			<td align="center" nowrap><?php
			$sid=$result->fields[StatesID];
			
			$RiSql="select * from imp_states where StatesID='$sid' ";
			$resultRiSql = $contact->Execute($RiSql) or die($contact->ErrorMsg());
			 echo $resultRiSql->fields['StatesName'] ; ?></td>
			 
			
			  
			  
       <td nowrap><strong><a href='<?php echo $_SERVER['PHP_SELF']."?city=$result->fields[city]";?>&show=product' class="blue_txt"> </a><b><?php echo $result->fields['city']; ?></b></strong></td>
...
<td align="center" nowrap><a href="city_add.php?city_id=<?php echo $result->fields['city_id'];?>"><img src="images/icon-edit.gif" border="0" alt="Edit" /></a>&nbsp;|&nbsp;<a href="city_view.php?del=yes&id=<?php echo $result->fields['city_id'];?>"><img src="images/icon-delete.gif" border="0" alt="Delete" /></a> </td>
			     
		  
   <td width="4%" align="center" nowrap><input name="bb[]" type="checkbox" id="bb[]" value="<?php echo $result->fields['city_id'];?>"></td>    
