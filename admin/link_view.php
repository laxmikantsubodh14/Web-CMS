...
if($_REQUEST['del']=='yes')
{
            $sqlDel="delete from imp_link where LinkId='$_REQUEST[id]'";
...
if(is_post_back()) {
	...
	if(is_array($bb)) {
		$bbb = implode(',', $bb);		
		if($_REQUEST['Delete']!='') {
			$sqlDel="delete from  imp_link where  LinkId in ($bbb)";
			$resultDel = executeQuery($sqlDel);
			...
			$sql = "update  imp_link  set Status= 'Y' where LinkId   in ($bbb)";
			...
			$sql = "update imp_link set Status= 'N' where LinkId in ($bbb)";
			$resultsql = executeQuery($sql);
...
$columns = "select * ";
$sql = " from  imp_link  where 1 ";
$sql = apply_filter($sql, $adm_login_id, $adm_login_id_filter,'LinkId');
$order_by == '' ? $order_by = 'LinkId' : true;
...
<table width="100%" border="0" cellpadding="4" cellspacing="1" class="tblBackColor">
  <tr bgcolor="#FFFFFF"><td><table width="100%" border="0" cellpadding="0" cellspacing="0" style="BACKGROUND-IMAGE: url(images/tablebgrep.gif)">
    <tr><td width="3%" class="pageHead"><img height="17" alt="" 
                ...
                vspace="3" /></td>
    <td width="97%" align="left"><div id="pageHead">Link List</div></td></tr></table>
</td></tr>
...
<tr><td height="250" valign="top"><br>
       <div align="right"> <a href="link_add.php">Add Link Information</a></div>
...
<?php } else{ ?>
  ...
  <table width="100%"  border="0" cellpadding="3" cellspacing="1" bgcolor="#d8d8d8" class="tableList" >
    <tr>
	      <th width="8%" nowrap> SNo. <?php echo sort_arrows('LinkId')?></th>
			 <th width="17%" nowrap> Link Title <?php echo sort_arrows('LinkTitle')?></th>
			 <th width="17%" nowrap> Link Url <?php echo sort_arrows('LinkUrl')?></th>
			 <th width="17%" nowrap> Category <?php echo sort_arrows('LinkCategory')?></th>
			 <th width="17%" nowrap> Description <?php echo sort_arrows('LinkDescription')?></th>
			 <th width="17%" nowrap> AddedDate <?php echo sort_arrows('LinkAddedDate')?></th>
...
?>   <tr class="<?php echo $css?>">
      	      <td nowrap><?php echo $LinkId;?> </td>
			  <td nowrap><?php echo $LinkTitle;?> </td>
			  <td nowrap><?php echo $LinkUrl;?> </td>
			  
	   <td align="center" nowrap><?php $regionSql=mysql_query("select * from emirates_link_category where CatID='$LinkCategory' order by CatID");
					  $regionRow=mysql_fetch_array($regionSql);
					  echo $regionRow['Category'];
					  ?></td>
			    <td nowrap><?php echo $LinkDescription;?> </td>
				<td nowrap><?php echo $LinkAddedDate;?> </td>
...
<td align="center" nowrap><a href="link_add.php?LinkId=<?php echo $LinkId;?>"><img src="images/icon-edit.gif" border="0" alt="Edit" /></a>&nbsp;|&nbsp;<a href="link_view.php?del=yes&id=<?php echo $LinkId;?>"><img src="images/icon-delete.gif" border="0" alt="Delete" /></a> </td>
			     
		  
   <td width="4%" align="center" nowrap><input name="bb[]" type="checkbox" id="bb[]" value="<?php echo $LinkId;?>"></td>    
 </tr><?php 
							   }
