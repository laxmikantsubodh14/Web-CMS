...
if($_REQUEST['del']=='yes')
{
            $sqlDel="delete from imp_filemanage where file_id='$_REQUEST[id]'";
...
if(is_post_back()) {
	...
	if(is_array($bb)) {
		$bbb = implode(',', $bb);		
		if($_REQUEST['Delete']!='') {
			$sqlDel="delete from  imp_filemanage where  file_id in ($bbb)";
			$resultDel = executeQuery($sqlDel);
			...
			$sql = "update  imp_filemanage  set Status= 'Y' where file_id   in ($bbb)";
			...
			$sql = "update imp_filemanage set Status= 'N' where file_id in ($bbb)";
			$resultsql = executeQuery($sql);
...
$columns = "select * ";
$sql = " from  imp_filemanage  where 1 ";
$sql = apply_filter($sql, $adm_login_id, $adm_login_id_filter,'file_id');
...
<table width="100%" border="0" cellpadding="4" cellspacing="1" class="tblBackColor">
  <tr bgcolor="#FFFFFF"><td><table width="100%" border="0" cellpadding="0" cellspacing="0" style="BACKGROUND-IMAGE: url(images/tablebgrep.gif)">
    <tr><td width="3%" class="pageHead"><img height="17" alt="" 
                ...
                vspace="3" /></td>
    <td width="97%" align="left"><div id="pageHead">File List</div></td></tr></table>
</td></tr>
...
<tr><td height="250" valign="top"><br>
       <div align="right"> <a href="file_add.php">Add File Information</a></div>
  ...
  <table width="100%"  border="0" cellpadding="3" cellspacing="1" bgcolor="#d8d8d8" class="tableList" >
    <tr>
	      <th width="8%" nowrap> SNo. <?php echo sort_arrows('file_id')?></th>
			 <th width="17%" nowrap> File Title <?php echo sort_arrows('file_title')?></th>
			 <th width="17%" nowrap> Customer <?php echo sort_arrows('CurrentDate')?></th>
						 <th width="17%" nowrap> AddedDate <?php echo sort_arrows('CurrentDate')?></th>
...
?>   <tr class="<?php echo $css?>">
      	      <td nowrap><?php echo $file_id;?> </td>
			  <td nowrap><?php echo $file_title;?> </td>
			  <td nowrap><?php echo $CurrentDate;?> </td>
...
<td align="center" nowrap><a href="file_add.php?file_id=<?php echo $file_id;?>"><img src="images/icon-edit.gif" border="0" alt="Edit" /></a>&nbsp;|&nbsp;<a href="file_view.php?del=yes&id=<?php echo $file_id;?>"><img src="images/icon-delete.gif" border="0" alt="Delete" /></a> </td>
			     
		  
   <td width="4%" align="center" nowrap><input name="bb[]" type="checkbox" id="bb[]" value="<?php echo $file_id;?>"></td>    
 </tr><?php 
							   }
