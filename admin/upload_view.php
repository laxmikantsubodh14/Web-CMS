...
if($_REQUEST['del']=='yes')
{
            $sqlDel="delete from imp_file where File_id='$_REQUEST[id]'";
...
if(is_post_back()) {
	...
	if(is_array($bb)) {
		$bbb = implode(',', $bb);		
		if($_REQUEST['Delete']!='') {
			$sqlDel="delete from imp_file where File_id in ($bbb)";
			$resultDel = $contact->Execute($sqlDel) or die($contact->ErrorMsg());
			$sess_msg=$sitemsgs[7];
			$_SESSION['sessionMsg']=$sess_msg;
			
		} else if($_REQUEST['Activate']!='') {
			 $sql = "update imp_file set Status= 'Y' where File_id  in ($bbb)";
			...
			$sql = "update imp_file set Status= 'N' where File_id in ($bbb)";
			$resultsql = $contact->Execute($sql) or die($contact->ErrorMsg());
...
$columns = "select * ";
$sql = " from  imp_file  where 1 ";
$sql = apply_filter($sql, $adm_login_id, $adm_login_id_filter,'Mem_id');
$order_by == '' ? $order_by = 'File_id' : true;
...
<table width="100%" border="0" cellpadding="4" cellspacing="1" class="tblBackColor">
  <tr bgcolor="#FFFFFF"><td><table width="100%" border="0" cellpadding="0" cellspacing="0" style="BACKGROUND-IMAGE: url(images/tablebgrep.gif)">
    <tr><td width="3%" class="pageHead"><img height="17" alt="" 
                ...
                vspace="3" /></td>
    <td width="97%" align="left"><div id="pageHead">File View</div></td></tr></table>
</td></tr>
...
<tr><td height="250" valign="top"><br>
       <div align="right"> <a href="upload_add.php">Add File </a></div>
...
<?php } else{ ?>
  ...
  <table width="100%"  border="0" cellpadding="3" cellspacing="1" bgcolor="#d8d8d8" class="tableList" >
          <tr>
	          <th width="8%" nowrap> SNo. <?php echo sort_arrows('File_id')?></th>
			 
              <th width="17%" nowrap>File Name <?php echo sort_arrows('File_name')?></th>
			 
			  <th width="17%" nowrap>View <?php echo sort_arrows('View')?></th>
			  <th width="17%" nowrap>Download <?php echo sort_arrows('Download')?></th>
...
while (!$result->EOF) 
{
	...
	if($result->fields['NEWS_STATUS']=="N") { $css="disable_row"; }
// select membres payment status

	$mem_id=$result->fields['Mem_id'];
	
	
?>   &nbsp;
         <tr class="<?php echo $css?>">
         <td nowrap><?php echo $result->fields['File_id']?> </td>
		
		<td align="center" nowrap><?php echo $result->fields['File_name']?></td>
		<?php if($result->fields['view']=='Y')
		{
		?>
		
		<td align="center" nowrap><?php echo "Yes"; ?></td>
		<?php 
		}
		else 
		{
		?>
		<td align="center" nowrap><?php echo "No"; ?></td>
		<?php 
		}
		?>
		<?php if($result->fields['download']=='Y')
		{
		?>
...
<td align="center" nowrap><a href="upload_add.php?File_id=<?php echo $result->fields['File_id'];?>"><img src="images/icon-edit.gif" border="0" alt="Edit" /></a>&nbsp;|&nbsp;<a href="upload_view.php?del=yes&id=<?php echo $result->fields['File_id'];?>"><img src="images/icon-delete.gif" border="0" alt="Delete" /></a> </td>
			     
		  
   <td width="4%" align="center" nowrap><input name="bb[]" type="checkbox" id="bb[]" value="<?php echo $result->fields['File_id'];?>"></td>    
        ...
        <tr>
          <td align="left"><table cellspacing="1" cellpadding="1" class="gray_txt" border="0">
              <!--<tr>
                ...
                <td>&nbsp;<span class="fieldsname">Disabled File</span></td>
              </tr>-->
          </table></td>
          <td align="right">
