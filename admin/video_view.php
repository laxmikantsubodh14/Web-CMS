...
if($_REQUEST['del']=='yes')
{
            $sqlDel="delete from imp_video where video_id='$_REQUEST[id]'";
...
if(is_post_back()) {
	...
	if(is_array($bb)) {
		$bbb = implode(',', $bb);		
		if($_REQUEST['Delete']!='') {
			$sqlDel="delete from imp_video where video_id in ($bbb)";
			$resultDel = $contact->Execute($sqlDel) or die($contact->ErrorMsg());
			$sess_msg=$sitemsgs[7];
			$_SESSION['sessionMsg']=$sess_msg;
			
		} else if($_REQUEST['Activate']!='') {
			 $sql = "update imp_video set Status= 'Y' where video_id  in ($bbb)";
			...
			$sql = "update imp_video set Status= 'N' where video_id in ($bbb)";
			$resultsql = $contact->Execute($sql) or die($contact->ErrorMsg());
...
<tr><td height="250" valign="top"><br>
       <div align="right"> <a href="video_add.php">Add Video </a></div>
...
<?php } else{ ?>
  ...
  <table width="100%"  border="0" cellpadding="3" cellspacing="1" bgcolor="#d8d8d8" class="tableList" >
          <tr>
	          <th width="8%" nowrap> SNo. <?php echo sort_arrows('video_id')?></th>
			 
              <th width="17%" nowrap>Video Name <?php echo sort_arrows('video_name')?></th>
...
?>   &nbsp;
         ...
         <td nowrap><?php echo $result->fields['video_id']?> </td>
		
		<td align="center" nowrap><?php echo $result->fields['video_name']?></td>
		
		<td align="center" nowrap><?php if($result->fields['Status'] =='Y') echo "Active"; else echo "InActive"; ?></td>
           
		   
<td align="center" nowrap><a href="video_add.php?video_id=<?php echo $result->fields['video_id'];?>"><img src="images/icon-edit.gif" border="0" alt="Edit" /></a>&nbsp;|&nbsp;<a href="video_view.php?del=yes&id=<?php echo $result->fields['video_id'];?>"><img src="images/icon-delete.gif" border="0" alt="Delete" /></a> </td>
			     
		  
   <td width="4%" align="center" nowrap><input name="bb[]" type="checkbox" id="bb[]" value="<?php echo $result->fields['video_id'];?>"></td>    
