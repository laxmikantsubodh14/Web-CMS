...
if($_REQUEST['del']=='yes')
{
            $sqlDel="delete from imp_customer where user_id='$_REQUEST[id]'";
...
if(is_post_back()) {
	...
	if(is_array($bb)) {
		$bbb = implode(',', $bb);		
		if($_REQUEST['Delete']!='') {
			$sqlDel="delete from imp_customer where user_id in ($bbb)";
			$resultDel = $contact->Execute($sqlDel) or die($contact->ErrorMsg());
			$sess_msg=$sitemsgs[7];
			$_SESSION['sessionMsg']=$sess_msg;
			
		} else if($_REQUEST['Activate']!='') {
			 $sql = "update imp_customer set user_isactive= 'Y' where user_id  in ($bbb)";
			...
			$sql = "update imp_customer set user_isactive= 'N' where user_id in ($bbb)";
			$resultsql = $contact->Execute($sql) or die($contact->ErrorMsg());
...
<table width="100%" border="0" cellpadding="4" cellspacing="1" class="tblBackColor">
  <tr bgcolor="#FFFFFF"><td><table width="100%" border="0" cellpadding="0" cellspacing="0" style="BACKGROUND-IMAGE: url(images/tablebgrep.gif)">
    <tr><td width="3%" class="pageHead"><img height="17" alt="" 
                ...
                vspace="3" /></td>
    <td width="97%" align="left"><div id="pageHead">Members  View</div></td></tr></table>
</td></tr>
...
<tr><td height="250" valign="top"><br>
       <div align="right"> <a href="members_add.php">Add Members </a></div>
        ...
        <tr>
          <td align="left"><table cellspacing="1" cellpadding="1" class="gray_txt" border="0">
              <tr>
                ...
                <td>&nbsp;<span class="fieldsname">Disabled  Members </span></td>
              </tr>
          </table></td>
          <td align="right">
