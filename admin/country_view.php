...
if($_REQUEST['del']=='yes')
{
            $sqlDel="delete from imp_country where country_id='$_REQUEST[id]'";
...
if(is_post_back()) {
	...
	if(is_array($bb)) {
		$bbb = implode(',', $bb);		
		if($_REQUEST['Delete']!='') {
			$sqlDel="delete from  imp_country where  country_id in ($bbb)";
			$resultDel = $contact->Execute($sqlDel) or die($contact->ErrorMsg());
			...
			$sql = "update  imp_country  set Status= 'Y' where country_id   in ($bbb)";
			...
			$sql = "update imp_country set Status= 'N' where country_id in ($bbb)";
			$resultsql = $contact->Execute($sql) or die($contact->ErrorMsg());
...
$start = intval($start);
$pagesize = intval($pagesize)==0?$pagesize=50:$pagesize;
$columns = "select * ";
$sql = " from  imp_country  where 1 ";
$sql = apply_filter($sql, $adm_login_id, $adm_login_id_filter,'country_id');
$order_by == '' ? $order_by = 'country' : true;
...
<title>Country view</title><div id="content">
...
<table width="100%" border="0" cellpadding="4" cellspacing="1" class="tblBackColor">
  <tr bgcolor="#FFFFFF"><td><table width="100%" border="0" cellpadding="0" cellspacing="0" style="BACKGROUND-IMAGE: url(images/tablebgrep.gif)">
    <tr><td width="3%" class="pageHead"><img height="17" alt="" 
                ...
                vspace="3" /></td>
    <td width="97%" align="left"><div id="pageHead">Country View</div></td></tr></table>
</td></tr>
...
<tr><td height="250" valign="top"><br>
       <div align="right"> <a href="country_add.php">Add Country Information</a></div>
...
<?php } else{ ?>
  ...
  <table width="100%"  border="0" cellpadding="3" cellspacing="1" bgcolor="#d8d8d8" class="tableList" >
    <tr>
	         <th width="8%" nowrap>ID. <?php echo sort_arrows('country_id')?></th>
             <th width="17%" nowrap> Country <?php echo sort_arrows('country')?></th>
             <th width="17%" nowrap> Country Code<?php echo sort_arrows('Code')?></th>
...
?>   <tr class="<?php echo $css?>">
      	      <td nowrap><?php echo $result->fields['country_id']?> </td>
			  
	
       <td nowrap><strong><?php echo $result->fields['country']; ?></strong></td>
	    <td nowrap><strong><?php echo $result->fields['Code']; ?></strong></td>
			<td align="center" nowrap><?php if($result->fields['Status'] =='Y') echo "Active"; else echo "InActive"; ?></td>
           
		   
<td align="center" nowrap>&nbsp;
<a href="country_add.php?country_id=<?php echo $result->fields['country_id'];?>"><img src="images/icon-edit.gif" border="0" alt="Edit" /></a>&nbsp;|&nbsp;<a href="country_view.php?del=yes&id=<?php echo $result->fields['country_id'];?>"><img src="images/icon-delete.gif" border="0" alt="Delete" /></a> </td>
<td width="4%" align="center" nowrap><input name="bb[]" type="checkbox" id="bb[]" value="<?php echo $result->fields['country_id'];?>"></td>    
        ...
        <tr>
          <td align="left"><table cellspacing="1" cellpadding="1" class="gray_txt" border="0">
              <tr>
                ...
                <td>&nbsp;<span class="fieldsname">Disabled Country. </span></td>
              </tr>
          </table></td>
