...
$error='';
		if($CatID!='') 
		{
			$sqlCheck="select * from imp_link_category_category where Category ='$Category' and CatID!='$CatID'";				 				            
			$rsCheck=executeQuery($sqlCheck);
			if(mysql_num_rows($rsCheck)==0)
			{								
				 $sqlUpdate="update imp_link_category  set Category='$Category',Status='$Status' where CatID='$CatID'";
				...
				location.href="linkcategory_view.php";
				</script>
				<?php 
		...
		{
			    $sqlCheck="select * from imp_link_category where Category ='$Category'";	
			    $rsCheck=executeQuery($sqlCheck);
				if(mysql_num_rows($rsCheck)==0)
				{
					$sqlInsert="insert into imp_link_category (Category,Status)
...
if(isset($CatID))
{
	$sqlBid="select * from  imp_link_category where CatID='".$CatID."'";
            ...
            <TABLE cellSpacing=0 cellPadding=0 width="98%" border=0>
              ...
              <TR>
                ...
                <TD class=blueheading align=left width="56%">Link Info </TD>
					...
					<td width="41%" align="right"><a href="linkcategory_view.php">
...
<form name="form1" method="post" action="linkcategory_add.php" onSubmit='return PostSearchForm();'>
				...
				<tr class="header_row">
				  ...
				  <td><input name="Category" type="text"  value="<?php echo $Category; ?>"></td></tr>
