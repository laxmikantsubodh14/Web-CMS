...
if($_REQUEST['del']=='yes')
    {
		$sqlDel="delete from imp_job_category  where CategoryID ='$_REQUEST[id]'";
...
if(is_post_back()) {
	...
	if(is_array($arr_unit_ids)) {
		...
		if($_REQUEST['Delete']!='') {
			$sqlDel="delete from imp_job_category  where CategoryID  in ($arr_unit_ids)";
			$rsDel=executeQuery($sqlDel);
			...
			$sql = "update imp_job_category set Status  = 'Y' where CategoryID  in ($arr_unit_ids)";
			...
			$sql = "update imp_job_category set Status  = 'N' where CategoryID  in ($arr_unit_ids)";
			mysql_query($sql) or die(db_error($sql));
...
$columns = "select * ";
$sql = " from imp_job_category where 1 ";
//$sql = apply_filter($sql, $adm_login_id, $adm_login_id_filter,'CategoryID ');
$order_by == '' ? $order_by = 'CategoryName' : true;
...
<table width="100%" border="0" cellpadding="4" cellspacing="1" class="tblBackColor">
  <tr bgcolor="#FFFFFF"><td><table width="100%" border="0" cellpadding="0" cellspacing="0" style="BACKGROUND-IMAGE: url(images/tablebgrep.gif)">
    <tr><td width="3%" class="pageHead"><img height="17" alt="" 
                ...
                vspace="3" /></td>
    <td width="97%" align="left"><div id="pageHead"> Job Category List </div></td></tr></table>
</td></tr>
...
<tr><td height="250" valign="top"><br>
       <div align="right"> <a href="jobcat_add.php">Add Job Category </a></div>
...
<div><span class="fieldsname">Records Per Page</span>:
  ...
  <table width="100%"  border="0" cellpadding="3" cellspacing="1" bgcolor="#d8d8d8" class="tableList" >
    <tr>
	      <!--<th width="8%" nowrap> SNo.<?php //echo sort_arrows('CategoryID')?></th>-->
            <th width="17%" nowrap> Category Name <?php echo sort_arrows('CategoryName')?></th>
...
?>   <tr class="<?php echo $css?>">
      	      <!--<td nowrap><?php //echo $CategoryID  ?> </td>-->
			  
           
		    <td nowrap align="center"><b><?php echo $CategoryName; ?></b></td>
			<td align="center" nowrap><?php if($Status=='Y') echo"Active"; else echo"InActive"; ?></td>
           
		   
<td align="center" nowrap><a href="jobcat_add.php?CategoryID=<?php echo $CategoryID;?>"><img src="images/icon-edit.gif" border="0" alt="Edit" /></a>&nbsp;|&nbsp;<a href="jobcat_list.php?del=yes&id=<?php echo $CategoryID;?>"><img src="images/icon-delete.gif" border="0" alt="Delete" /></a> </td>
			
   <td width="4%" align="center" nowrap><input name="arr_unit_ids[]" type="checkbox" id="arr_unit_ids[]" value="<?php echo $CategoryID ?>"></td>    
 </tr><?php }
?>
    <tr align="right">
        ...
        <tr>
          <td align="left"><table cellspacing="1" cellpadding="1" class="gray_txt" border="0">
              <tr>
                ...
                <td>&nbsp;<span class="fieldsname">Disabled Job </span></td>
              </tr>
          </table></td>
          <td align="right">
