	...
	for($i=0; $i<count($postID); $i++)
	{
		...
		if($options[$i] == 'Y')
		{
		  ...
		  if(checkUniquenessOfString('imp_emails','EMAILS',"$EMAILS"))
				...
				{
					$insertmail="insert into imp_emails (EMAILS, SUBMITTIME, APPROVE) values ('$EMAILS', NOW(), 'Y')";
					//mysql_query($insertmail) or die("Could not insert email");
					$contact->Execute($insertmail) or die($contact->ErrorMsg());
					$delemails="Delete from imp_tempemails where EMAIL_ID=$postID[$i]";
					//mysql_query($delemails) or die("Could not Approve emails");	
    ...
    <tr align="right">
      <td colspan="10" bgcolor="#FFFFFF"><table width="100%">
        <tr>
          ...
          <td align="right"><input type="submit" name="Activate" value="Approve" class="button" onClick="onApprove();" />
